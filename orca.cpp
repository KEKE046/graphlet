...
struct _GraphletCounter {

	inline static const int no[]  = {0,0,1,4,15,73};
	inline static const int eno[] = {0,0,0,2,12,68};
...
#define common2_get(x) (((common2_it=common2.find(x))!=common2.end())?(common2_it->second):0)
	...
	int64_t n, m; // n = number of nodes, m = number of edges
	...
	bool adjacent(int x, int y) { return binary_search(adj[x].begin(),adj[x].end(),y); }
	...
	int getEdgeId(int x, int y) { return inc[x][lower_bound(adj[x].begin(),adj[x].end(),y)-adj[x].begin()].second; }

	vector<vector<int64_t>> orbit; // orbit[x][o] - how many times does node x participate in orbit o
	vector<vector<int64_t>> eorbit; // eorbit[x][o] - how many times does node x participate in edge orbit o

	_GraphletCounter(int64_t n, int64_t m, int64_t * _u, int64_t * _v)
		: n(n), m(m), deg(n), edges(m), adj(n), inc(n) {
		unordered_map<PAIR, int, hash_PAIR> edge_set;
		for(int i = 0; i < m; i++) {
			auto u = _u[i], v = _v[i];
			if(u > v) swap(u, v);
			if(u > n || v > n) {
				throw runtime_error("edge " + to_string(i) + ", index larger than n.");
			}
			if(u == v) {
				throw runtime_error("self loop: " + to_string(i) + "");
			}
			if(edge_set.count(PAIR(u, v))) {
				throw runtime_error("duplicated edges: " + to_string(i) + " and " + to_string(edge_set[PAIR(u, v)]) + " (" + to_string(u) + "," + to_string(v) + ")");
			}
			edge_set[PAIR(u, v)] = i;
			// edge_set.insert({PAIR(u, v), i});
			deg[u]++; deg[v]++;
			edges[i]=PAIR(u, v);
			adj[u].push_back(v); adj[v].push_back(u);
			inc[u].push_back(PII(v, i));
			inc[v].push_back(PII(u, i));
		}
		for (int i=0;i<n;i++) {
	...
	void ecount5() {
		...
		for (int x=0;x<n;x++) {
			...
			for (int nx=0;nx<deg[x];nx++) {
				...
				int64_t f_66=0, f_65=0, f_62=0, f_61=0, f_60=0, f_51=0, f_50=0; // 11
				int64_t f_64=0, f_58=0, f_55=0, f_48=0, f_41=0, f_35=0; // 10
				int64_t f_63=0, f_59=0, f_57=0, f_54=0, f_53=0, f_52=0, f_47=0, f_40=0, f_39=0, f_34=0, f_33=0; // 9
				int64_t f_45=0, f_36=0, f_26=0, f_23=0, f_19=0; // 7
				int64_t f_49=0, f_38=0, f_37=0, f_32=0, f_25=0, f_22=0, f_18=0; // 6
				int64_t f_56=0, f_46=0, f_44=0, f_43=0, f_42=0, f_31=0, f_30=0; // 5
				int64_t f_27=0, f_17=0, f_15=0; // 4
				int64_t f_20=0, f_16=0, f_13=0; // 3
				int64_t f_29=0, f_28=0, f_24=0, f_21=0, f_14=0, f_12=0; // 2
...
#undef common2_get
	...
	void write(int graphlet_size, int64_t * ret) {
		int ptr = 0;
		for(int i = 0; i < n; i++) {
			for(int j = 0; j < no[graphlet_size]; j++) {
				ret[ptr++] = orbit[i][j];
			}
		}
	}
	void ewrite(int graphlet_size, int64_t * ret) {
		int ptr = 0;
		for(int i = 0; i < m; i++) {
			for(int j = 0; j < eno[graphlet_size]; j++) {
				ret[ptr++] = orbit[i][j];
			}
		}
	}

	void count(int graphlet_size, bool node_graphlet, int64_t * ret) {
		if(node_graphlet) {
			orbit.resize(n);
			for(auto & v: orbit) v.resize(73);
			if(graphlet_size == 4) count4();
			if(graphlet_size == 5) count5();
			write(graphlet_size, ret);
			orbit.clear();
		}
		else {
			eorbit.resize(m);
			for(auto & v: eorbit) v.resize(68);
			if(graphlet_size == 4) ecount4();
			if(graphlet_size == 5) ecount5();
			ewrite(graphlet_size, ret);
			eorbit.clear();
		}
	}
};
